...
services=("checkoutservice" "currencyservice" "loadgenerator" "productcatalogservice" "recommendationservice" "shippingservice")

for service in "${services[@]}"; do
    echo "Creating branch and Jenkins file for $service..."
    
    # Switch to main and create branch
    git checkout main
    git checkout -b $service
    
    # Create Jenkins file content
    cat > jenkins-files/jenkisfile-$service << JENKINS_EOF
pipeline {
    agent any

    environment {
        AWS_REGION = "us-east-1"
        SOURCE_IMAGE = "us-central1-docker.pkg.dev/google-samples/microservices-demo/$service"
        ECR_URI = "183611507646.dkr.ecr.us-east-1.amazonaws.com"
        REPO_NAME = "microservices-demo/$service"
                ...
                script {
                    env.LATEST_TAG = sh(
                        script: '''
                            ...
                            TAG_JSON=\$(curl -s https://us-central1-docker.pkg.dev/v2/google-samples/microservices-demo/$service/tags/list)
                ...
                withCredentials([[\$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-creds']]) {
                    sh '''
                        ...
                        if aws ecr describe-images --repository-name \${REPO_NAME} --region \${AWS_REGION} \\
                            ...
                            echo "⬇️ Pulling image from GCP and pushing to ECR..."
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'github-token', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) {
                    sh '''
                        ...
                        git commit -m "Update $service Helm chart to \$LATEST_TAG" || echo "No changes to commit"
...
JENKINS_EOF
    ...
    # Add and commit
    git add jenkins-files/jenkisfile-$service
    git commit -m "Add $service Jenkins pipeline"
    
    echo "✅ Created $service branch and Jenkins file"
done
