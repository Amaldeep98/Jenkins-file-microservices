pipeline {
    ...
    environment {
        ...
        SOURCE_IMAGE = "us-central1-docker.pkg.dev/google-samples/microservices-demo/cartservice"
        ECR_URI = "183611507646.dkr.ecr.us-east-1.amazonaws.com"
        REPO_NAME = "microservices-demo/cartservice"
        GITHUB_REPO = "https://github.com/Amaldeep98/microservice-helm-charts.git"
        CHART_DIR = "helm-charts/cartservice"
    }
                ...
                script {
                    env.LATEST_TAG = sh(
                        script: '''
                            ...
                            TAG_JSON=$(curl -s https://us-central1-docker.pkg.dev/v2/google-samples/microservices-demo/cartservice/tags/list)
...
"
                        ...
                        git commit -m "Update cartservice Helm chart to $LATEST_TAG" || echo "No changes to commit"
